...
if ($use_auth) {
    if (isset($_SESSION['logged'], $auth_users[$_SESSION['logged']])) {
        ...
        if (isset($auth_users[$_POST['fm_usr']]) && $_POST['fm_pwd'] === $auth_users[$_POST['fm_usr']]) {
            ...
            fm_set_msg('Вы вошли как ' . $_POST['fm_usr']);
            fm_redirect(FM_SELF_URL . '?p=');
        } else {
            unset($_SESSION['logged']);
            fm_set_msg('Неверный пароль', 'error');
            fm_redirect(FM_SELF_URL);
        }
    } else {
        // Form
        ...
        <div class="path">
            <form action="" method="post" style="margin:10px;text-align:center">
                <input name="fm_usr" value="" placeholder="Логин" required>
                <input type="password" name="fm_pwd" value="" placeholder="Пароль" required>
...
if (isset($_GET['del'])) {
    ...
    if ($del != '' && $del != '..' && $del != '.') {
        ...
        if (fm_rdelete($path . '/' . $del)) {
            $msg = $is_dir ? 'Папка <b>%s</b> удалена' : 'Файл <b>%s</b> удалён';
            fm_set_msg(sprintf($msg, fm_enc($del)));
        } else {
            $msg = $is_dir ? 'Папка <b>%s</b> не удалена' : 'Файл <b>%s</b> не удалён';
            fm_set_msg(sprintf($msg, fm_enc($del)), 'error');
        }
    } else {
        fm_set_msg('Неверное имя файла или папки', 'error');
    }
...
if (isset($_GET['new'])) {
    ...
    if ($new != '' && $new != '..' && $new != '.') {
        ...
        if (fm_mkdir($path . '/' . $new, false) === true) {
            fm_set_msg(sprintf('Папка <b>%s</b> создана', fm_enc($new)));
        } elseif (fm_mkdir($path . '/' . $new, false) === $path . '/' . $new) {
            fm_set_msg(sprintf('Папка <b>%s</b> уже существует', fm_enc($new)), 'alert');
        } else {
            fm_set_msg(sprintf('Папка <b>%s</b> не создана', fm_enc($new)), 'error');
        }
    } else {
        fm_set_msg('Неверное имя папки', 'error');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_GET['copy'], $_GET['finish'])) {
    ...
    // empty path
    if ($copy == '') {
        fm_set_msg('Исходный путь не определен', 'error');
        fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
    }
    // abs path from
    ...
    if ($from != $dest) {
        ...
        if ($move) {
            ...
            if ($rename) {
                fm_set_msg(sprintf('Перемещено из <b>%s</b> в <b>%s</b>', fm_enc($copy), fm_enc($msg_from)));
            } elseif ($rename === null) {
                fm_set_msg('Файл или папка с таким путем уже существует', 'alert');
            } else {
                fm_set_msg(sprintf('Ошибка при перемещении из <b>%s</b> в <b>%s</b>', fm_enc($copy), fm_enc($msg_from)), 'error');
            }
        } else {
            if (fm_rcopy($from, $dest)) {
                fm_set_msg(sprintf('Скопировано из <b>%s</b> в <b>%s</b>', fm_enc($copy), fm_enc($msg_from)));
            } else {
                fm_set_msg(sprintf('Ошибка при копировании из <b>%s</b> в <b>%s</b>', fm_enc($copy), fm_enc($msg_from)), 'error');
            }
        }
    } else {
        fm_set_msg('Пути должны быть не равны', 'alert');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_POST['file'], $_POST['copy_to'], $_POST['finish'])) {
    ...
    if (!is_dir($copy_to_path)) {
        if (!fm_mkdir($copy_to_path, true)) {
            fm_set_msg('Невозможно создать папку назначения', 'error');
            fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
        }
    }
    // move?
    ...
    if (is_array($files) && count($files)) {
        ...
        if ($errors == 0) {
            $msg = $move ? 'Перемещение выбранных файлов и папок' : 'Копирование выбранных файлов и папок';
            fm_set_msg($msg);
        } else {
            $msg = $move ? 'Ошибка при перемещении элементов' : 'Ошибка при копировании элементов';
            fm_set_msg($msg, 'error');
        }
    } else {
        fm_set_msg('Ничего не выбрано', 'alert');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_GET['ren'], $_GET['to'])) {
    ...
    if ($old != '' && $new != '') {
        if (fm_rename($path . '/' . $old, $path . '/' . $new)) {
            fm_set_msg(sprintf('Переименовано из <b>%s</b> в <b>%s</b>', fm_enc($old), fm_enc($new)));
        } else {
            fm_set_msg(sprintf('Ошибка при переименовании из <b>%s</b> в <b>%s</b>', fm_enc($old), fm_enc($new)), 'error');
        }
    } else {
        fm_set_msg('Имена не установлены', 'error');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_GET['dl'])) {
    ...
    if ($dl != '' && is_file($path . '/' . $dl)) {
        ...
        fm_set_msg('Файл не найден', 'error');
        fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
    }
}
...
if (isset($_POST['upl'])) {
    ...
    if ($errors == 0 && $uploads > 0) {
        fm_set_msg(sprintf('Все файлы загружены в <b>%s</b>', fm_enc($path)));
    } elseif ($errors == 0 && $uploads == 0) {
        fm_set_msg('Ничего не загружено', 'alert');
    } else {
        fm_set_msg(sprintf('Ошибка при загрузке файлов. Загруженные файлы: %s', $uploads), 'error');
    }
...
if (isset($_POST['group'], $_POST['delete'])) {
    ...
    if (is_array($files) && count($files)) {
        ...
        if ($errors == 0) {
            fm_set_msg('Выбранные файлы и папка удалены');
        } else {
            fm_set_msg('Ошибка при удалении элементов', 'error');
        }
    } else {
        fm_set_msg('Ничего не выбрано', 'alert');
    }
...
if (isset($_POST['group'], $_POST['zip'])) {
    ...
    if (!class_exists('ZipArchive')) {
        fm_set_msg('Операции с архивами недоступны', 'error');
        fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
    }
    ...
    if (!empty($files)) {
        ...
        if ($res) {
            fm_set_msg(sprintf('Архив <b>%s</b> создан', fm_enc($zipname)));
        } else {
            fm_set_msg('Архив не создан', 'error');
        }
    } else {
        fm_set_msg('Ничего не выбрано', 'alert');
    }
...
if (isset($_GET['unzip'])) {
    ...
    if ($unzip != '' && is_file($path . '/' . $unzip)) {
        ...
        if ($res) {
            fm_set_msg('Архив распакован');
        } else {
            fm_set_msg('Архив не распакован', 'error');
        }
...
if (isset($_POST['chmod']) && !FM_IS_WIN) {
    ...
    if (@chmod($path . '/' . $file, $mode)) {
        fm_set_msg('Разрешения изменены');
    } else {
        fm_set_msg('Разрешения не изменены', 'error');
    }
...
if (isset($_GET['upload'])) {
    ...
    <div class="path">
        ...
        <p class="break-word">Папка назначения: <?php echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . FM_PATH)) ?></p>
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <br>
            <p>
                <button class="btn"><i class="icon-apply"></i> Загрузить</button> &nbsp;
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>"><i class="icon-cancel"></i> Отмена</a></b>
            </p>
        </form>
    </div>
    <?php
...
if (isset($_POST['copy'])) {
    ...
    <div class="path">
        ...
        <form action="" method="post">
            ...
            ?>
            <p class="break-word">Файлы: <b><?php echo implode('</b>, <b>', $copy_files_enc) ?></b></p>
            <p class="break-word">Исходная папка: <?php echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . FM_PATH)) ?><br>
                <label for="inp_copy_to">Папка назначения:</label>
            ...
            <p>
                <button class="btn"><i class="icon-apply"></i> Скопировать</button> &nbsp;
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>"><i class="icon-cancel"></i> Отменить</a></b>
            </p>
        </form>
    </div>
    <?php
...
if (isset($_GET['copy']) && !isset($_GET['finish'])) {
    ...
    <div class="path">
        <p><b>Копирование</b></p>
        <p class="break-word">
            Исходный путь: <?php echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . $copy)) ?><br>
            Папка назначения: <?php echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . FM_PATH)) ?>
        </p>
        <p>
            <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode($copy) ?>&amp;finish=1"><i class="icon-apply"></i> Скопировать</a></b> &nbsp;
            <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode($copy) ?>&amp;finish=1&amp;move=1"><i class="icon-apply"></i> Переместить</a></b> &nbsp;
            <b><a href="?p=<?php echo urlencode(FM_PATH) ?>"><i class="icon-cancel"></i> Отменить</a></b>
        </p>
        <p><i>Выберите папку:</i></p>
...
if (isset($_GET['view'])) {
    ...
    <div class="path">
        ...
        <p class="break-word">
            Полный путь: <?php echo fm_enc(fm_convert_win($file_path)) ?><br>
            Размер файла: <?php echo fm_get_filesize($filesize) ?><?php if ($filesize >= 1000): ?> (<?php echo sprintf('%s bytes', $filesize) ?>)<?php endif; ?><br>
            MIME-тип: <?php echo $mime_type ?><br>
            <?php
            // ZIP info
            if ($is_zip && $filenames !== false) {
                ...
                Общий размер: <?php echo fm_get_filesize($total_uncomp) ?><br>
                Размер в архиве: <?php echo fm_get_filesize($total_comp) ?><br>
                Сжатие: <?php echo round(($total_comp / $total_uncomp) * 100) ?>%<br>
                <?php
            }
            // Image info
            if ($is_image) {
                $image_size = getimagesize($file_path);
                echo 'Размеры изображения: ' . (isset($image_size[0]) ? $image_size[0] : '0') . ' x ' . (isset($image_size[1]) ? $image_size[1] : '0') . '<br>';
            }
            // Text info
            if ($is_text) {
                ...
                echo 'Набор символов: ' . ($is_utf8 ? 'utf-8' : '8 bit') . '<br>';
            }
            ?>
        </p>
        <p>
            <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($file) ?>"><i class="icon-download"></i> Скачать</a></b> &nbsp;
            <b><a href="<?php echo fm_enc($file_url) ?>" target="_blank"><i class="icon-chain"></i> Открыть</a></b> &nbsp;
            <?php
            // ZIP actions
            if ($is_zip && $filenames !== false) {
                $zip_name = pathinfo($file_path, PATHINFO_FILENAME);
                ?>
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;unzip=<?php echo urlencode($file) ?>"><i class="icon-apply"></i> Распаковать</a></b> &nbsp;
            ...
            ?>
            <b><a href="?p=<?php echo urlencode(FM_PATH) ?>"><i class="icon-goback"></i> Назад</a></b>
        </p>
        <?php
        if ($is_zip) {
            // ZIP content
            if ($filenames !== false) {
                ...
                echo '<p>Ошибка при получении информации архива</p>';
            }
        } elseif ($is_image) {
            // Image content
...
if (isset($_GET['chmod']) && !FM_IS_WIN) {
    ...
    ?>
    <div class="path">
        <p><b>Изменить разрешения</b></p>
        <p>
            Полный путь: <?php echo fm_enc($file_path) ?><br>
        </p>
        <form action="" method="post">
            <input type="hidden" name="p" value="<?php echo fm_enc(FM_PATH) ?>">
            ...
            <table class="compact-table">
                ...
                <tr>
                    <td style="text-align: right"><b>Читать</b></td>
                    <td><label><input type="checkbox" name="ur" value="1"<?php echo ($mode & 00400) ? ' checked' : '' ?>></label></td>
                    <td><label><input type="checkbox" name="gr" value="1"<?php echo ($mode & 00040) ? ' checked' : '' ?>></label></td>
                    <td><label><input type="checkbox" name="or" value="1"<?php echo ($mode & 00004) ? ' checked' : '' ?>></label></td>
                </tr>
                <tr>
                    <td style="text-align: right"><b>Писать</b></td>
                    <td><label><input type="checkbox" name="uw" value="1"<?php echo ($mode & 00200) ? ' checked' : '' ?>></label></td>
                    <td><label><input type="checkbox" name="gw" value="1"<?php echo ($mode & 00020) ? ' checked' : '' ?>></label></td>
                    <td><label><input type="checkbox" name="ow" value="1"<?php echo ($mode & 00002) ? ' checked' : '' ?>></label></td>
                </tr>
                <tr>
                    <td style="text-align: right"><b>Выполнять</b></td>
            ...
            <p>
                <button class="btn"><i class="icon-apply"></i> Изменить</button> &nbsp;
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>"><i class="icon-cancel"></i> Отмена</a></b>
            </p>
...
<th>Название</th><th style="width:10%">Размер</th>
<th style="width:12%">Модифицированно</th>
<?php if (!FM_IS_WIN): ?><th style="width:6%">Права доступа</th><th style="width:10%">Владелец</th><?php endif; ?>
...
<td>Папка</td><td><?php echo $modif ?></td>
<?php if (!FM_IS_WIN): ?>
<td><a title="Change Permissions" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;chmod=<?php echo urlencode($f) ?>"><?php echo $perms ?></a></td>
<td><?php echo fm_enc($owner['name'] . ':' . $group['name']) ?></td>
<?php endif; ?>
<td>
<a title="Удалить" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('Удалить папку?');"><i class="icon-cross"></i></a>
<a title="Переименовать" href="#" onclick="rename('<?php echo fm_enc(FM_PATH) ?>', '<?php echo fm_enc($f) ?>');return false;"><i class="icon-rename"></i></a>
<a title="Скопировать в..." href="?p=&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="icon-copy"></i></a>
<a title="Прямая ссылка" href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f . '/') ?>" target="_blank"><i class="icon-chain"></i></a>
</td></tr>
    <?php
    flush();
}
...
<?php endif; ?>
<td>
<a title="Удалить" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('Удалить файл?');"><i class="icon-cross"></i></a>
<a title="Переименовать" href="#" onclick="rename('<?php echo fm_enc(FM_PATH) ?>', '<?php echo fm_enc($f) ?>');return false;"><i class="icon-rename"></i></a>
<a title="Скопировать в..." href="?p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="icon-copy"></i></a>
<a title="Прямая ссылка" href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f) ?>" target="_blank"><i class="icon-chain"></i></a>
<a title="Скачать" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($f) ?>"><i class="icon-download"></i></a>
</td></tr>
    <?php
    flush();
}
...
if (empty($folders) && empty($files)) {
    ?>
<tr><td></td><td colspan="<?php echo !FM_IS_WIN ? '6' : '4' ?>"><em>Папка пуста</em></td></tr>
<?php
} else {
    ?>
<tr><td class="gray"></td><td class="gray" colspan="<?php echo !FM_IS_WIN ? '6' : '4' ?>">
Полный размер: <span title="<?php printf('%s bytes', $all_files_size) ?>"><?php echo fm_get_filesize($all_files_size) ?></span>,
Файлов: <?php echo $num_files ?>,
Папок: <?php echo $num_folders ?>
</td></tr>
<?php
}
?>
</table>
<p class="path"><a href="#" onclick="select_all();return false;"><i class="icon-checkbox"></i> Выбрать всё</a> &nbsp;
<a href="#" onclick="unselect_all();return false;"><i class="icon-checkbox_uncheck"></i> Снять всё</a> &nbsp;
<a href="#" onclick="invert_all();return false;"><i class="icon-checkbox_invert"></i> Наоборот</a></p>
<p><input type="submit" name="delete" value="Удалить" onclick="return confirm('Удалить выбранные файлы и папки?')">
<input type="submit" name="zip" value="Сжать" onclick="return confirm('Создать архив?')">
<input type="submit" name="copy" value="Скопировать"></p>
</form>
